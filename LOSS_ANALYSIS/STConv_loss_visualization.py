...
if __name__ == '__main__':
    ...
    _, means, stds, _, _, valid_input, valid_target, test_input, test_target = \
        load_pems_spd_data(input_steps=num_timesteps_input, output_steps=num_timesteps_output, FORECAST_GAP=FORECAST_GAP)
    ...
    net = STConv(valid_input.shape[1],
                test_input.shape[3],
                num_timesteps_input,
                num_timesteps_output, Ks=Ks, Kt=Kt).to(device=device)
    net.eval()
    ...
    mean_detector_mse = np.mean(loss*stds**2, axis=1)
